...
当用户显式触发“全面思考”、`comprehensive-thinking`、`深度理解`、`系统性分析`、`帮我想清楚`、`从根上看`、`策略判断`、`架构判断`或同等高责任复杂判断请求时，除非用户明确要求简短、只要结论或不要展开过程，否则输出必须采用五重审视结构。
...
1. 行业大师的理论体系
2. 领域公认经典著作，涉及到商业领域必须选择《商业模式新生代》的商业画布
3. 长期被验证的实践框架、原始论文、官方规范、一线成熟系统的架构说明、工程手册
4. 领域专家的行业文章
...
在 `VIRTUAL X 2.0` 的金融系统、实盘交易、账本、回测、订单、营收、客户资产、QMT Bridge、页面数据真值等问题中，全面思考必须和项目正式规则叠加：

- 金融真值问题必须先保护真实、完整、可追溯，不允许用兜底冒充正常。
- 页面问题必须检查真实数据合理性，不只检查页面能打开。
- 实盘问题必须区分交易核心链、行情链、历史链、诊断链，不把局部超时说成整体不可用。
- 架构问题必须从业务树、主链边界、未来实盘演进路径一起判断。
